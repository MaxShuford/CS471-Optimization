...
/**
 * @file algorithms.h
 * @brief Differential Evolution interface.
 *
 * This header declares the differential evolution (DE) algorithm used to
 * evaluate benchmark problems. The DE implementation reports per-iteration
 * fitness values, the best fitness found, and total execution time.
 */

double diff_evo(const Problem* p, int m, int iters, int pop_size, double F, double CR, double lambda, int generations,
                        double lower, double upper, double* fitness_out, double* best_out, 
                        double* time_ms_out, DEStrategy strategy);

/**
 * @file algorithms.h
 * @brief Particle Swarm Optimization interface.
 *
 * This header declares the particle swarm optimization (PSO) algorithm
 * used to evaluate benchmark problems. The PSO implementation reports
 * per-iteration fitness values, the best fitness found, and total
 * execution time.
 */
double p_swarm(const Problem* p, int m, int iters, int swarm_size, double w, int generations,
                    double c1, double c2, double lower, double upper,
