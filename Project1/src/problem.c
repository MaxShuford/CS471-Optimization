...
{
    double mn = -100.0, mx = 100.0;
    if (!p) { *out_min = mn; *out_max = mx; return; }

    switch (p->type) {
        case PROB_SCHWEFEL: mn = -512.0; mx = 512.0; break;
        case PROB_DEJONG1: mn = -100.0; mx = 100.0; break;
        case PROB_ROSENBROCK: mn = -100.0; mx = 100.0; break;
        case PROB_RASTRIGIN: mn = -30.0; mx = 30.0; break;
        case PROB_GRIEWANGK: mn = -500.0; mx = 500.0; break;
        case PROB_SINE_ENV_SINE_WAVE: mn = -30.0; mx = 30.0; break;
        case PROB_STRETCH_V_SINE_WAVE: mn = -30.0; mx = 30.0; break;
        case PROB_ACKLEY_ONE: mn = -32.0; mx = 32.0; break;
        case PROB_ACKLEY_TWO: mn = -32.0; mx = 32.0; break;
        case PROB_EGG_HOLDER: mn = -500.0; mx = 500.0; break;
        default: break;
    }
        ...
        case PROB_ACKLEY_TWO: {
            ...
            for (int i = 0; i < m - 1; i++) {
                ...
                double term = 20.0 + exp(1.0)
                              - 20.0 * exp(0.2 * a)
                              - exp(0.5 * (cos(2.0 * M_PI * x[i]) + cos(2.0 * M_PI * x[i + 1])));
                sum += term;
            }
            return sum;
        }
